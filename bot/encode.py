...
# Khởi tạo bot
bot = telebot.TeleBot("7201356785:AAG_NZqh_6xCcshO--JfNU_K-N8VKVuK2D8")  # Thay bằng token bot của bạn
...
def start(message):
	bot.reply_to(message, "Gửi /encode 1, 2 hoặc 3, sau đó gửi file để encode!")
...
def handle_file(message):
	...
	try:
		...
		subprocess.run(
			['python3', '/encode/Sakura.py', "-f", input_file, '-o', output_file, "-m", mode],
			check=True
		)
